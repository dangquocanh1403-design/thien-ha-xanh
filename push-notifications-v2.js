...
async function syncStatus(){
  ...
  try{
    ...
    if(permission&&subscribed&&subscriptionId){
      if(window.currentUserId&&window.currentUserId!==lastUser){lastUser=window.currentUserId;await sdk.login(lastUser)}
      setStatus(`Đã bật trên thiết bị này · ${subscriptionId.slice(0,8)}…`,true);button.textContent="✓ Đã bật"
    }
...
button.addEventListener("click",async()=>{
  ...
  try{
    ...
    if(sdk.User.PushSubscription.id&&window.currentUserId){await sdk.login(window.currentUserId);lastUser=window.currentUserId}
    await syncStatus();
